...
export const getDomain = (host) => {
  ...
  const defaultDomain = "hellothisdefault.domain";
...
// Utility function to check if the domain is a test link
const isTestLink = (domain) => {
  return ["localhost", "vercel", "amplifyapp", "amplifytest"].some((sub) =>
    domain.includes(sub)
  );
};
