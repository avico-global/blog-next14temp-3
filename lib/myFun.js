...
import { downloadAllImages, extractImageData } from "./downloadImages";
// import { downloadAllImages, extractImageData } from "./downloadImages";
...
export async function downloadImages({ domain, project_id }) {
  return null;
  // if (fs.existsSync("public/images")) {
  //   return; // Return false if the directory does not exist
  // }
  try {
    const data = await callBackendApi({ domain, type: "" });
    const imagesUrls = await extractImageData(data);
    console.log("👊 ~ downloadImages ~ imagesUrls:", imagesUrls);
    const imgsURLs = await downloadAllImages({
      result: imagesUrls,
      project_id,
    });
    console.log("👊 ~ downloadImages ~ imgsURLs:", imgsURLs);

    return JSON.stringify(imgsURLs);
  } catch (err) {
