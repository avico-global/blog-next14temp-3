...
export const downloadImagesIfNeeded = async (imageUrls) => {
  ...
  const downloadedImages = []; // Array to store the mappings
  ...
  for (const { type, url } of imageUrls) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      try {
        ...
        const buffer = await response.arrayBuffer(); // Use await here
        fs.writeFileSync(filePath, Buffer.from(buffer));
        console.log(`Image downloaded and saved: ${fileName}`);
      } catch (err) {
        console.error(`Error saving image ${url}:`, err);
        continue; // Skip adding to mappings if there's an error
      }
    }

    // Determine the correct image path (relative or absolute URL)
    const imagePath = url.includes(process.env.NEXT_PUBLIC_SITE_MANAGER)
      ? `/images/${fileName}`
      : url;

    // Add the mapping for the current image
    downloadedImages.push({ type, image: imagePath });
  }

  // Write the mappings to a .js file that exports the object
