...
export const getDomain = (host) => {
  let domain = "picoriverafiredamagerestoration.us";
  if (
    host &&
...
export const callBackendApi = async ({ domain, query = null, type = "" }) => {
  ...
  if (!isTestLink && !project_id && domain) {
    ...
    baseURL = `${process.env.NEXT_PUBLIC_SITE_MANAGER}/api/public/default_project_data/data`;
  }
  ...
  try {
    ...
    console.log("🚀 ~ callBackendApi ~ err:", err);
    return {
      error: {
        status: err?.status,
